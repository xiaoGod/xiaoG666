...
//    private static final String CACHE_IMAGE_DIR = "/devices";
    ...
    private static String getIMIEStatus(Context context) {
        ...
        if (ActivityCompat.checkSelfPermission(MainApp.getAppContext(), Manifest.permission.READ_PHONE_STATE) != PackageManager.PERMISSION_GRANTED) {
...
//                file.mkdirs();
//            }
            File cropdir = new File(CACHE_IMAGE_DIR(context));
            if (!cropdir.exists()) {
                cropdir.mkdirs();
            }
            mCropFile = new File(cropdir, DEVICES_FILE_NAME); // 用当前时间给取得的图片命名
        } else {
            File cropdir = new File(context.getFilesDir(), CACHE_IMAGE_DIR(context));
            if (!cropdir.exists()) {
                cropdir.mkdir();
            }
