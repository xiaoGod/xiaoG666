...
static bool js_engine_FileUtils_getDataSizeFromFile(se::State& s)
{
    cocos2d::FileUtils* cobj = (cocos2d::FileUtils*)s.nativeThisObject();
    SE_PRECONDITION2(cobj, false, "js_engine_FileUtils_getDataSizeFromFile : Invalid Native Object");
    const auto& args = s.args();
    size_t argc = args.size();
    CC_UNUSED bool ok = true;
    if (argc == 1) {
        std::string arg0;
        ok &= seval_to_std_string(args[0], &arg0);
        SE_PRECONDITION2(ok, false, "js_engine_FileUtils_getDataSizeFromFile : Error processing arguments");
        int size = 0;
        if(cocos2d::FileUtils::getInstance()->isFileExist(arg0)) {
            cocos2d::Data result = cobj->getDataFromFile(arg0);
            size = result.getSize();
        }
        ok &= int32_to_seval(size, &s.rval());
        SE_PRECONDITION2(ok, false, "js_engine_FileUtils_getDataSizeFromFile : Error processing arguments");
        return true;
    }
    SE_REPORT_ERROR("wrong number of arguments: %d, was expecting %d", (int)argc, 1);
...
SE_BIND_FUNC(js_engine_FileUtils_getDataSizeFromFile)
...
static bool js_engine_FileUtils_getBundlePath(se::State& s)
{
    cocos2d::FileUtils* cobj = (cocos2d::FileUtils*)s.nativeThisObject();
    SE_PRECONDITION2(cobj, false, "js_engine_FileUtils_getBundlePath : Invalid Native Object");
    const auto& args = s.args();
    size_t argc = args.size();
    CC_UNUSED bool ok = true;
    if (argc == 0) {
        std::string result = cobj->getBundlePath();
        ok &= std_string_to_seval(result, &s.rval());
        SE_PRECONDITION2(ok, false, "js_engine_FileUtils_getBundlePath : Error processing arguments");
        return true;
    }
    SE_REPORT_ERROR("wrong number of arguments: %d, was expecting %d", (int)argc, 0);
    return false;
}
SE_BIND_FUNC(js_engine_FileUtils_getBundlePath)
...
bool js_register_engine_FileUtils(se::Object* obj)
{
    ...
    cls->defineFunction("getDataSizeFromFile", _SE(js_engine_FileUtils_getDataSizeFromFile));
...
static bool js_engine_Device_getVersion(se::State& s)
{
    const auto& args = s.args();
    size_t argc = args.size();
    CC_UNUSED bool ok = true;
    if (argc == 0) {
        std::string result = cocos2d::Device::getVersion();
        ok &= std_string_to_seval(result, &s.rval());
        SE_PRECONDITION2(ok, false, "js_engine_Device_getVersion : Error processing arguments");
        return true;
    }
    SE_REPORT_ERROR("wrong number of arguments: %d, was expecting %d", (int)argc, 0);
    return false;
}
SE_BIND_FUNC(js_engine_Device_getVersion)

static bool js_engine_Device_getBundleId(se::State& s)
{
    const auto& args = s.args();
    size_t argc = args.size();
    CC_UNUSED bool ok = true;
    if (argc == 0) {
        std::string result = cocos2d::Device::getBundleId();
        ok &= std_string_to_seval(result, &s.rval());
        SE_PRECONDITION2(ok, false, "js_engine_Device_getBundleId : Error processing arguments");
        return true;
    }
    SE_REPORT_ERROR("wrong number of arguments: %d, was expecting %d", (int)argc, 0);
    return false;
}
SE_BIND_FUNC(js_engine_Device_getBundleId)

static bool js_engine_Device_getChannelId(se::State& s)
{
    const auto& args = s.args();
    size_t argc = args.size();
    CC_UNUSED bool ok = true;
    if (argc == 0) {
        std::string result = cocos2d::Device::getChannelId();
        ok &= std_string_to_seval(result, &s.rval());
        SE_PRECONDITION2(ok, false, "js_engine_Device_getChannelId : Error processing arguments");
        return true;
    }
    SE_REPORT_ERROR("wrong number of arguments: %d, was expecting %d", (int)argc, 0);
    return false;
}
SE_BIND_FUNC(js_engine_Device_getChannelId)

static bool js_engine_Device_getInviteCode(se::State& s)
{
    const auto& args = s.args();
    size_t argc = args.size();
    CC_UNUSED bool ok = true;
    if (argc == 0) {
        std::string result = cocos2d::Device::getInviteCode();
        ok &= std_string_to_seval(result, &s.rval());
        SE_PRECONDITION2(ok, false, "js_engine_Device_getInviteCode : Error processing arguments");
        return true;
    }
    SE_REPORT_ERROR("wrong number of arguments: %d, was expecting %d", (int)argc, 0);
    return false;
}
SE_BIND_FUNC(js_engine_Device_getInviteCode)

static bool js_engine_Device_getAppName(se::State& s)
{
    const auto& args = s.args();
    size_t argc = args.size();
    CC_UNUSED bool ok = true;
    if (argc == 0) {
        std::string result = cocos2d::Device::getAppName();
        ok &= std_string_to_seval(result, &s.rval());
        SE_PRECONDITION2(ok, false, "js_engine_Device_getAppName : Error processing arguments");
        return true;
    }
    SE_REPORT_ERROR("wrong number of arguments: %d, was expecting %d", (int)argc, 0);
    return false;
}
SE_BIND_FUNC(js_engine_Device_getAppName)

static bool js_engine_Device_getMachineId(se::State& s)
{
    const auto& args = s.args();
    size_t argc = args.size();
    CC_UNUSED bool ok = true;
    if (argc == 0) {
        std::string result = cocos2d::Device::getMachineId();
        ok &= std_string_to_seval(result, &s.rval());
        SE_PRECONDITION2(ok, false, "js_engine_Device_getMachineId : Error processing arguments");
        return true;
    }
    SE_REPORT_ERROR("wrong number of arguments: %d, was expecting %d", (int)argc, 0);
    return false;
}
SE_BIND_FUNC(js_engine_Device_getMachineId)

static bool js_engine_Device_getAndroidId(se::State& s)
{
    const auto& args = s.args();
    size_t argc = args.size();
    CC_UNUSED bool ok = true;
    if (argc == 0) {
        std::string result = cocos2d::Device::getAndroidId();
        ok &= std_string_to_seval(result, &s.rval());
        SE_PRECONDITION2(ok, false, "js_engine_Device_getAndroidId : Error processing arguments");
        return true;
    }
    SE_REPORT_ERROR("wrong number of arguments: %d, was expecting %d", (int)argc, 0);
    return false;
}
SE_BIND_FUNC(js_engine_Device_getAndroidId)

static bool js_engine_Device_getGoogleAdId(se::State& s)
{
    const auto& args = s.args();
    size_t argc = args.size();
    CC_UNUSED bool ok = true;
    if (argc == 0) {
        std::string result = cocos2d::Device::getGoogleAdId();
        ok &= std_string_to_seval(result, &s.rval());
        SE_PRECONDITION2(ok, false, "js_engine_Device_getGoogleAdId : Error processing arguments");
        return true;
    }
    SE_REPORT_ERROR("wrong number of arguments: %d, was expecting %d", (int)argc, 0);
    return false;
}
SE_BIND_FUNC(js_engine_Device_getGoogleAdId)

static bool js_engine_Device_isOnlineApp(se::State& s)
{
    const auto& args = s.args();
    size_t argc = args.size();
    CC_UNUSED bool ok = true;
    if (argc == 0) {
        bool result = cocos2d::Device::isOnlineApp();
        ok &= boolean_to_seval(result, &s.rval());
        SE_PRECONDITION2(ok, false, "js_engine_Device_isOnlineApp : Error processing arguments");
        return true;
    }
    SE_REPORT_ERROR("wrong number of arguments: %d, was expecting %d", (int)argc, 0);
    return false;
}
SE_BIND_FUNC(js_engine_Device_isOnlineApp)

static bool js_engine_Device_checkLocationPermission(se::State& s)
{
    const auto& args = s.args();
    size_t argc = args.size();
    CC_UNUSED bool ok = true;
    if (argc == 0) {
        bool result = cocos2d::Device::checkLocationPermission();
        ok &= boolean_to_seval(result, &s.rval());
        SE_PRECONDITION2(ok, false, "js_engine_Device_checkLocationPermission : Error processing arguments");
        return true;
    }
    SE_REPORT_ERROR("wrong number of arguments: %d, was expecting %d", (int)argc, 0);
    return false;
}
SE_BIND_FUNC(js_engine_Device_checkLocationPermission)

static bool js_engine_Device_getOnlineAppSwitchTime(se::State& s)
{
    const auto& args = s.args();
    size_t argc = args.size();
    CC_UNUSED bool ok = true;
    if (argc == 0) {
        std::string result = cocos2d::Device::getOnlineAppSwitchTime();
        ok &= std_string_to_seval(result, &s.rval());
        SE_PRECONDITION2(ok, false, "js_engine_Device_getOnlineAppSwitchTime : Error processing arguments");
        return true;
    }
    SE_REPORT_ERROR("wrong number of arguments: %d, was expecting %d", (int)argc, 0);
    return false;
...
static bool js_engine_Device_copyTextToClipboard(se::State& s)
{
    ...
    if (argc == 1) {
        ...
        SE_PRECONDITION2(ok, false, "js_engine_Device_copyTextToClipboard : Error processing arguments");
        cocos2d::Device::copyTextToClipboard(text);
        return true;
    }
    SE_REPORT_ERROR("wrong number of arguments: %d, was expecting %d", (int)argc, 0);
    return false;
}
SE_BIND_FUNC(js_engine_Device_copyTextToClipboard)

static bool js_engine_Device_showShare(se::State& s)
{
    const auto& args = s.args();
    size_t argc = args.size();
    CC_UNUSED bool ok = true;
    if (argc == 1) {
        std::string text;
        ok &= seval_to_std_string(args[0], &text);
        SE_PRECONDITION2(ok, false, "js_engine_Device_showShare : Error processing arguments");
        cocos2d::Device::showShare(text);
        return true;
    }
    SE_REPORT_ERROR("wrong number of arguments: %d, was expecting %d", (int)argc, 0);
    return false;
}
SE_BIND_FUNC(js_engine_Device_showShare)

static bool js_engine_Device_showToast(se::State& s)
{
    const auto& args = s.args();
    size_t argc = args.size();
    CC_UNUSED bool ok = true;
    if (argc == 1) {
        std::string text;
        ok &= seval_to_std_string(args[0], &text);
        SE_PRECONDITION2(ok, false, "js_engine_Device_showToast : Error processing arguments");
        cocos2d::Device::showToast(text);
        return true;
    }
    SE_REPORT_ERROR("wrong number of arguments: %d, was expecting %d", (int)argc, 0);
    return false;
}
SE_BIND_FUNC(js_engine_Device_showToast)
...
bool js_register_engine_Device(se::Object* obj)
{
     ...
     *  static std::string getVersion();
        ...
        static std::string getGoogleAdId();
        static bool isOnlineApp();
        static void copyTextToClipboard(const std::string& text);
        static void showShare(const std::string& share_text);
    ...
    cls->defineStaticFunction("getChannelId", _SE(js_engine_Device_getChannelId));
    cls->defineStaticFunction("getInviteCode", _SE(js_engine_Device_getInviteCode));
    cls->defineStaticFunction("getAppName", _SE(js_engine_Device_getAppName));
    cls->defineStaticFunction("getMachineId", _SE(js_engine_Device_getMachineId));
    cls->defineStaticFunction("getAndroidId", _SE(js_engine_Device_getAndroidId));
    cls->defineStaticFunction("getGoogleAdId", _SE(js_engine_Device_getGoogleAdId));
    cls->defineStaticFunction("isOnlineApp", _SE(js_engine_Device_isOnlineApp));
    cls->defineStaticFunction("copyTextToClipboard", _SE(js_engine_Device_copyTextToClipboard));
    cls->defineStaticFunction("showShare", _SE(js_engine_Device_showShare));
